...
public class CharacterInputController : MonoBehaviour
{
    ...
    public void CurrentLanePosition(int lane)
    {
        lane = m_CurrentLane;
    }
	...
	public void Begin()
	{
        ...
        character.animator2.SetBool(s_DeadHash, false);
    ...
    public void CleanConsumable(string go)
    {
        for (int i = 0; i < m_ActiveConsumables.Count; ++i)
        {
            if (m_ActiveConsumables[i].GetConsumableType() == Consumable.ConsumableType.INVINCIBILITY)
            {
               
                m_ActiveConsumables[i].Ended(this);
                Destroy(m_ActiveConsumables[i].gameObject);
                m_ActiveConsumables.Remove(m_ActiveConsumables[i]);
                gs.ClearPowerupINV();
            }
        }
    ...
    public void StartRunning()
    {
        ...
        if (character.animator2)
        {
            character.animator2.Play(s_RunStartHash);
            character.animator2.SetBool(s_MovingHash, true);
        }
    }
    ...
    public void StopMoving()
    {
        ...
        if (character.animator2)
        {
            character.animator2.SetBool(s_MovingHash, false);
        }
    }
    ...
    protected void Update()
    {
        ...
        {
            if (Mathf.Abs(Input.mousePosition.x) > Mathf.Abs(Input.mousePosition.y))
            {
        ...
        if (PlayerPrefs.GetInt("MoveType") == 2)
        {
            if ((Input.GetTouch(0).phase == TouchPhase.Began))
            {
                if (Mathf.Abs(Input.GetTouch(0).position.x) > Mathf.Abs(Input.GetTouch(0).position.y))
                {
        ...
        else if (PlayerPrefs.GetInt("MoveType") == 1)
        {
        ...
        if(m_Jumping)
        {
			if (trackManager.isMoving)
			{
				...
				if (ratio >= 1.0f)
				{
					...
					character.animator2.SetBool(s_JumpingHash, false);
				}
				else
				{
    ...
    public void Jump()
    {
        if (!m_Jumping)
        {
            ...
            character.animator2.SetFloat(s_JumpingSpeedHash, animSpeed);
            character.animator2.SetBool(s_JumpingHash, true);
	...
	public void Slide()
	{
		if (!m_Sliding)
		{
            ...
            character.animator2.SetFloat(s_JumpingSpeedHash, animSpeed);
            character.animator2.SetBool(s_SlidingHash, true);
	...
	public void StopSliding()
	{
		if (m_Sliding)
		{
			...
			character.animator2.SetBool(s_SlidingHash, false);
			m_Sliding = false;
    ...
    public void StopConsumable(Consumable c)
    {
        c.Ended(this);
    }
}
