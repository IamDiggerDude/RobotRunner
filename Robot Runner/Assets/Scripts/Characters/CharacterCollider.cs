...
public class CharacterCollider : MonoBehaviour
{
    ...
    [HideInInspector]
	public List<GameObject> magnetPowerUp= new List<GameObject>();

    protected bool m_Invincible;
    protected bool m_Armor;
    ...
    protected void Update()
	{
        ...
        for (int i = 0; i < magnetPowerUp.Count; ++i)
		{
            magnetPowerUp[i].transform.position = Vector3.MoveTowards(magnetPowerUp[i].transform.position, transform.position, k_MagnetSpeed * Time.deltaTime);
		}

        if (m_Invincible)
        {
            foreach (GameObject n in character.form1)
            {
                n.SetActive(false);
            }
            foreach (GameObject n in character.form2)
            {
                n.SetActive(true);
            }
        }
    ...
    protected bool colliderWithWall;
    ...
    protected IEnumerator InvincibleTimer(float timer)
    {
        ...
        while (time < timer && m_Invincible && m_Armor)
            {
