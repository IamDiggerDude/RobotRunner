...
public class GameState : AState
{
    ...
    public void ClearPowerupINV()
    {
        ...
        for (int i = 0; i < m_PowerupIcons.Count; ++i)
        {
            if (m_PowerupIcons[i] != null)
            {
                Debug.Log("Concret power up: " + m_PowerupIcons[i].linkedConsumable.GetConsumableType());
                if (m_PowerupIcons[i].linkedConsumable.GetConsumableType() == Consumable.ConsumableType.INVINCIBILITY)
                {
                    Destroy(m_PowerupIcons[i].gameObject);
                }
            }
