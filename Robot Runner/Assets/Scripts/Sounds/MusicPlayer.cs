...
public class MusicPlayer : MonoBehaviour
{
    ...
    public IEnumerator RestartAllStems()
    {
        ...
        /*for (int i = 0; i < stems.Length; ++i)
        {
            stems[i].source.volume = stems[i].startingSpeedRatio <= 0.0f ? maxVolume * (Convert.ToInt64(PlayerPrefs.GetInt("Volume")) / 100f) : 0.0f;
        }*/

        AudioSource[] AudioSources = FindObjectsOfType<AudioSource>();
        for (int i = 0; i < AudioSources.Length; i++)
        {
            AudioSources[i].volume = Convert.ToInt64(PlayerPrefs.GetInt("Volume")) / 100f;
        }
    }
    ...
    public void UpdateVolumes(float currentSpeedRatio)
    {
        ...
        /*for(int i = 0; i < stems.Length; ++i)
        {
            ...
            //stems[i].source.volume = (Mathf.MoveTowards(stems[i].source.volume, target, fadeSpeed * Time.deltaTime))* (Convert.ToInt64(PlayerPrefs.GetInt("Volume")) / 100f);
            stems[i].source.volume = (Convert.ToInt64(PlayerPrefs.GetInt("Volume")) / 100f);
        }*/
    ...
    public void VolumeChange(float volume)
    {
        ...
        for (int i = 0; i < stems.Length; ++i)
        {
            stems[i].source.volume = volume/100;
        }
    }
}
